...
def submitted():
    ...
    if timeOut > timeIn: 
        ...
        cost = time_play * co
        profit = bill - cost
        new_mark = {'username': [username.get()], 'check in date':[calIn], 'check in':[timeIn], 'check out date':[calOut], 'check out':[timeOut], 'fee':[bill], 'cost':[cost], 'profit':[profit]} #setหัวตารางที่กำลังจะทำ
        mark_df = pd.DataFrame(new_mark) #สร้างตารางใน python
        frame = pd.concat([frame,mark_df], ignore_index=True) #เอาตารางในไพทอนไปเป็นตารางใน excel
...
window = Tk() #ฟังชันก์สร้างหน้าต่าง pop up
...
time_in = Combobox(window, values=time) #ตัวที่เลื่อนลงมาเลือกได้
